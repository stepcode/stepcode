...
//  private:
    const TypeDescriptor *local_definition_(const TypeDescriptor *td)
	{ _local_definition = td; }
    const Express_id original_id_(const Express_id& ei)
	{ _original_id = ei; }

    // non-sdai
    const Express_id new_id_(const Express_id& ni)
	{ _new_id = ni; }
};
...
class InverseADItr
{
  ...
  public:
    InverseADItr (const InverseAttrDescriptorList &iadList);
    virtual ~InverseADItr();
...
//	StringAggregate  & DerivedAttr()	{ return *_derivedAttr; }
	...
	const InverseAttrDescriptorList& InverseAttr() const
	        { return _inverseAttr; }
