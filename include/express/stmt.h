...
#define STMT_destroy(x) MEM_destroy(&STMT_fl,(Freelist *)x)

#define ALIAS_new()     (struct Alias_ *)MEM_new(&ALIAS_fl)
#define ALIAS_destroy(x)        MEM_destroy(&ALIAS_fl,(Freelist *)x)
#define ASSIGN_new()        (struct Assignment_ *)MEM_new(&ASSIGN_fl)
#define ASSIGN_destroy(x)       MEM_destroy(&ASSIGN_fl,(Freelist *)x)
#define CASE_new()      (struct Case_Statement_ *)MEM_new(&CASE_fl)
#define CASE_destroy(x)     MEM_destroy(&CASE_fl,(Freelist *)x)
#define COMP_STMT_new()     (struct Compound_Statement_ *)MEM_new(&COMP_STMT_fl)
#define COMP_STMT_destroy(x)        MEM_destroy(&COMP_STMT_fl,(Freelist *)x)
#define COND_new()      (struct Conditional_ *)MEM_new(&COND_fl)
#define COND_destroy(x)     MEM_destroy(&COND_fl,(Freelist *)x)
#define LOOP_new()      (struct Loop_ *)MEM_new(&LOOP_fl)
#define LOOP_destroy(x)     MEM_destroy(&LOOP_fl,(Freelist *)x)
#define PCALL_new()     (struct Procedure_Call_ *)MEM_new(&PCALL_fl)
#define PCALL_destroy(x)    MEM_destroy(&PCALL_fl,(Freelist *)x)
#define RET_new()       (struct Return_Statement_ *)MEM_new(&RET_fl)
#define RET_destroy(x)      MEM_destroy(&RET_fl,(Freelist *)x)
